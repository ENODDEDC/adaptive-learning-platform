    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <p style={{ margin: 0, fontSize: '13px', fontWeight: 600, color: '#f4f4f5', lineHeight: '1.4' }}>
            {toast.title}
          </p>
          {toast.body && (
            <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#a1a1aa', lineHeight: '1.5' }}>
              {toast.body}
            </p>
          )}
          {toast.hint && (
            <p style={{ margin: '6px 0 0', fontSize: '11px', color: '#71717a', lineHeight: '1.4' }}>
              {toast.hint}
            </p>
          )}
        </div>
        ...
        <button
          ...
          style={{
            ...
            padding: '2px', color: '#52525b', flexShrink: 0,
...
const FileUpload = ({ onFilesReady, initialFiles = [], folder = 'classwork', courseId = null }) => {
  ...
  const showToast = (title, body, hint) => setToast({ title, body, hint });
  ...
  useEffect(() => {
    ...
    if (JSON.stringify(allFiles) !== JSON.stringify(previousFiles)) {
      if (onFilesReady) onFilesReady(allFiles);
  ...
  const VIDEO_EXTENSIONS = new Set(['mp4', 'webm', 'mov', 'avi', 'mkv', 'ogv', 'wmv', 'flv', '3gp', 'mpeg', 'mpg']);
  ...
  const handleFileChange = async (e) => {
    ...
    if (videoFiles.length > 0) {
      showToast(
        'Video files not allowed here',
        `Use the "Video Link" tab to add videos via URL.`,
        `Blocked: ${videoFiles.map(f => f.name).join(', ')}`
      );
      e.target.value = '';
      return;
    }
  ...
  const uploadToBackblaze = useCallback(async (filesToUpload) => {
    ...
    try {
      ...
      setFiles(prev => prev.filter(file => !filesToUpload.some(u => u.name === file.name)));
      ...
      <div className="flex flex-col items-center justify-center w-full">
        ...
        >
          <input
            type="file"
            multiple
            accept=".pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx,.txt,.md,.jpg,.jpeg,.png,.gif,.webp,.svg,.mp3,.wav,.ogg,.aac"
          ...
          <div className="w-full mt-4">
            ...
            <ul className="space-y-2">
              {files.map((file, index) => (
                <li key={index} className="flex items-center justify-between p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                  ...
                  {uploadProgress[file.name] !== undefined && (
                    <div className="w-16 bg-gray-200 rounded-full h-2 mr-2">
                      <div className="bg-blue-600 h-2 rounded-full transition-all duration-300" style={{ width: `${uploadProgress[file.name]}%` }} />
                    </div>
                  )}
                  <button onClick={() => handleRemoveFile(file.name, false)} className="ml-2 text-red-500 hover:text-red-700 p-1" disabled={isUploading}>
          ...
          <div className="w-full mt-4">
            ...
            <ul className="space-y-2">
              {uploadedFiles.map((file, index) => (
                <li key={file._id || file.key || index} className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-md">
                  <div className="flex items-center flex-1 min-w-0">
                    ...
                    {file.url && (
                      <a href={file.url} target="_blank" rel="noopener noreferrer" className="ml-2 text-blue-500 hover:text-blue-700">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <button onClick={() => handleRemoveFile(file._id || file.key, true)} className="ml-2 text-red-500 hover:text-red-700 p-1">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
