...
export default function TextToDocsClient() {
  ...
  useEffect(() => {
    ...
    if (urlPrompt) {
      // Valid access with prompt - mark as authorized and store in session
      setPrompt(urlPrompt);
      setHasValidAccess(true);
      sessionStorage.setItem('textToDocsAccess', 'true');
      ...
      const hasSessionAccess = sessionStorage.getItem('textToDocsAccess') === 'true';
      
      if (hasSessionAccess) {
        setHasValidAccess(true);
      } else {
        // No prompt parameter and no session access means direct access - show access denied
        ...
        <div className="max-w-md mx-auto text-center">
          <div className="bg-white rounded-2xl shadow-lg p-8 border border-red-200">
            ...
            <p className="text-gray-600 mb-6">
              This page can only be accessed through the AI Assistant on the home page. Please use the &ldquo;Text to Docs&rdquo; feature from there.
            </p>
            <button
