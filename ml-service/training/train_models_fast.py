"""
Fast Training Script for FSLSM Classification
Target: 96%+ accuracy with faster training

Optimization:
- Uses simple train/val/test split (no cross-validation)
- Optimized hyperparameters (no grid search)
...
def train_dimension_model_fast(X_train, y_train, X_val, y_val, dimension_name):
    """Train XGBoost model with optimized hyperparameters (no grid search)"""
    print(f"\n🎯 Training model for: {dimension_name}")
    
    # Optimized hyperparameters (found through previous experiments)
    # These parameters balance accuracy and speed
    params = {
...
def main():
    ...
    print("=" * 70)
    print("🚀 FAST FSLSM Model Training - Target: 96% Accuracy")
    print("=" * 70)
    print("\n⚡ Validation Technique: Simple Train/Val/Test Split")
    print("   (No cross-validation for speed)")
    ...
    # Save scaler
    scaler_path = models_dir / 'scaler_fast.pkl'
    ...
    dimensions = {
        'activeReflective': 'active_reflective_fast',
        'sensingIntuitive': 'sensing_intuitive_fast',
        'visualVerbal': 'visual_verbal_fast',
        'sequentialGlobal': 'sequential_global_fast'
    }
    ...
    for dim_label, dim_file in dimensions.items():
        ...
        # Train model (fast - no grid search)
        model, val_mae, val_r2 = train_dimension_model_fast(
    ...
    print(f"\n⚡ Validation Technique Used: Simple Train/Val/Test Split")
    print(f"   - Fast: No cross-validation")
    print(f"   - Reliable: Large enough dataset (2500 samples)")
    print(f"   - Industry standard for large datasets")
